...
function generateData () {
    const Start = faker.date.future({years: 2,refDate: new Date()}).toISOString().split('T')[0];
    ...
    const data =  {
        ...
        eventCategory: faker.lorem.words(),
        eventType: faker.helpers.arrayElement(["Ausstellung", "Auktion", "Messe", "Vortrag", "Festival"]),
        img: faker.image.urlLoremFlickr({ keywords: 'music', height: 480, width: 640 }),
        description: faker.lorem.words(),
        homepage: faker.internet.url(),
        dateStart: usedStart,
        dateEnd: End,
        timeStart: faker.date.future().toISOString().split('T')[1].split('.')[0].split(':').slice(0, 2).join(':'),
        timeEnd: faker.date.future().toISOString().split('T')[1].split('.')[0].split(':').slice(0, 2).join(':'),
        // info about venue
        venueName: faker.lorem.words(),
        venueType: faker.helpers.arrayElement(["Museum", "Galerie", "Messe", "Auktionshaus", "Akademie"]),
        ...
        artistImg: faker.image.urlLoremFlickr({ keywords: 'music', height: 480, width: 640 }),
        }
    return data;
    }
...
try {
 for (let i = 0; i < 10; i++) { // change the number of users here
    const userid = await createData();
    console.log(`User ${i} created ` + userid)
    for (let i = 0; i < 100; i++){ // change the number of events here
